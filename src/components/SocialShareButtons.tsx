...
export default function SocialShareButtons({
  ...
  const shareText = isEn
    ? `Check out this free printable coloring page: ${title}`
    : `Bekijk deze gratis printbare kleurplaat: ${title}`;
    ...
    <div className={styles.shareContainer}>
      ...
      <div className={styles.buttonRow}>
        ...
        <button
          ...
          type="button"
          aria-label={copied ? (isEn ? 'Link copied' : 'Link gekopieerd') : (isEn ? 'Share link' : 'Deel link')}
        >
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
            <rect x="9" y="9" width="13" height="13" rx="2" ry="2"/>
            <path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/>
          </svg>
          <span>{copied ? (isEn ? 'Copied!' : 'Gekopieerd!') : (isEn ? 'Share Link' : 'Deel Link')}</span>
        </button>
      </div>
    </div>
  );
