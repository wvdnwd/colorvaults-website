...
export default function SearchFilterBar({
 ...
 placeholder={
 isEn
 ?'Search by title, tag, or topic...':'Zoek op titel, tag of onderwerp...'}
 ...
 aria-label="Clear query">
      ...
      <div style={{ display:'flex', alignItems:'center', gap:'0.45rem', overflowX:'auto', padding:'0.35rem 0 0.85rem', scrollbarWidth:'none'}}>
        ...
        {[
          ...
          <button
            key={pill.q}
            type="button"onClick={() => onFilterChange({ query: pill.q })}
            style={{
 ...
 <button
 type="button"className={styles.mobileToggleBtn}
 ...
 <button
 type="button"className={`${styles.filterChipBtn} ${!difficulty ? styles.chipActive :''}`}
 ...
 <button
 key={key}
 type="button"className={`${styles.filterChipBtn} ${difficulty === key ? styles.chipActive :''}`}
 ...
 <div className={styles.selectGroup}>
 <button
 type="button"className={`${styles.filterChipBtn} ${!age ? styles.chipActive :''}`}
 ...
 <button
 key={key}
 type="button"className={`${styles.filterChipBtn} ${age === key ? styles.chipActive :''}`}
 ...
 ️ &ldquo;{query}&rdquo;
 <button
 type="button"onClick={() => onFilterChange({ query:''})}
 ...
 <button
 type="button"onClick={() => onFilterChange({ difficulty:''})}
 className={styles.chipRemoveBtn}
 ...
 <button
 type="button"onClick={() => onFilterChange({ age:''})}
 className={styles.chipRemoveBtn}
 ...
 {selectedThemeTitle}
 <button
 type="button"onClick={() => onFilterChange({ theme:''})}
 className={styles.chipRemoveBtn}
