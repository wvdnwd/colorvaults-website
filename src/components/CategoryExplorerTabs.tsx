  ...
  return (
    <div className={styles.explorerWrapper}>
      ...
      <div className={styles.tabScrollContainer} role="tablist"aria-label="Category filter">
        <button
          type="button"role="tab"aria-selected={selectedHub ==='all'}
      ...
      <div className={styles.bottomCtaRow} style={{ display:'flex', justifyContent:'center', gap:'1rem', flexWrap:'wrap', marginTop:'2.5rem'}}>
        {hasMore && (
          <button
            type="button"onClick={handleToggleShowAll}
            className="btn-primary"style={{ fontWeight: 800, cursor:'pointer', padding:'0.85rem 1.75rem'}}
          >
            {showAll
        ...
        {selectedHub !=='all'&& activeHubObj && (
          <Link
            ...
            className="btn-secondary"style={{ fontWeight: 800, padding:'0.85rem 1.75rem'}}
          >
            {isEn 
              ?`Open ${activeHubObj.title} Hub →`:`Open ${activeHubObj.title} Overzicht →`}
          </Link>
        )}
