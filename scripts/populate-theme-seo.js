...
const enThemesPath = path.join(__dirname, '../src/data/en/themes.json');
const nlThemesPath = path.join(__dirname, '../src/data/nl/themes.json');

const enThemes = JSON.parse(fs.readFileSync(enThemesPath, 'utf8'));
const nlThemes = JSON.parse(fs.readFileSync(nlThemesPath, 'utf8'));
...
const SPECIFIC_SEO = {
  animals: {
    ...
    enBottom: `<h2>Who Are Our Animals Coloring Pages Suitable For?</h2>
...
RELATED_LINKS`,
    nlBottom: `<h2>Voor wie zijn onze Dieren Kleurplaten geschikt?</h2>
...
RELATED_LINKS`
  ...
  mandalas: {
    ...
    enBottom: `<h2>Mindfulness & Relaxation with Mandala Coloring Pages</h2>
...
RELATED_LINKS`,
    nlBottom: `<h2>Ontspanning & Mindfulness met Mandala Kleurplaten</h2>
...
RELATED_LINKS`
  ...
  'disney-princesses': {
    ...
    enBottom: `<h2>Magical Disney Princess Coloring Pages for Every Fan</h2>
...
RELATED_LINKS`,
    nlBottom: `<h2>Betoverende Disney Prinsessen Kleurplaten voor Jong en Oud</h2>
...
function generateSeoForTheme(theme, lang, allThemes) {
  const isEn = lang === 'en';
  const hubSlug = theme.parentHub;
  const themeSlug = theme.slug;
  const title = theme.title;
  const override = SPECIFIC_SEO[themeSlug];
  ...
  let relatedSlugs = override?.relatedSlugs || [];
  if (relatedSlugs.length === 0) {
    relatedSlugs = allThemes
      .filter(t => t.parentHub === hubSlug && t.slug !== themeSlug)
      .slice(0, 3)
      .map(t => t.slug);
  }
  ...
  const relatedLinksList = relatedSlugs
    .map(slug => {
      const found = allThemes.find(t => t.slug === slug);
      const tTitle = found ? found.title : slug.replace(/-/g, ' ').replace(/\b\w/g, c => c.toUpperCase());
      return `<li><a href="/${lang}/${hubSlug}/${slug}"><strong>${tTitle} ${isEn ? 'Coloring Pages' : 'Kleurplaten'}</strong></a></li>`;
    })
    .join('\n');

  const relatedLinksHtml = relatedLinksList ? `<ul>\n${relatedLinksList}\n</ul>` : '';

  let seoIntro = '';
  let seoContent = '';

  if (override) {
    seoIntro = isEn ? override.enIntro : override.nlIntro;
    const rawBottom = isEn ? override.enBottom : override.nlBottom;
    seoContent = rawBottom
    ...
    if (isEn) {
      seoIntro = `Discover our vast collection of free printable ${title} coloring pages! Perfect for kids, toddlers, teens, and adults. Download high-resolution PDF and image templates ready for instant printing at home or in school without any registration.`;
      seoContent = `<h2>Free Printable ${title} Coloring Pages for All Ages</h2>
<p>Our high-quality ${title} coloring collection offers creative inspiration for everyone. Designed in crisp line vector format, these templates are optimized for easy A4 printing and digital coloring apps.</p>

<h3>Age Groups & Difficulty Levels</h3>
<p>Find the best ${title} coloring page suited for your skill level:</p>
<ul>
...
<p>Looking for <em>free printable ${title} coloring pages PDF</em> or <em>easy ${title} coloring sheets for toddlers</em>? ColorVaults provides clean, instant downloads for endless hours of coloring fun.</p>

${relatedLinksHtml ? `<h3>Explore Related Categories</h3>\n<p>Check out these popular coloring categories:</p>\n${relatedLinksHtml}` : ''}`;
    } else {
      seoIntro = `Ontdek onze uitgebreide verzameling gratis ${title} kleurplaten om direct uit te printen! Geschikt voor peuters, kinderen, tieners en volwassenen. Download hoge resolutie PDF- en afbeeldingensjablonen gratis voor thuis of in de klas.`;
      seoContent = `<h2>Gratis Printbare ${title} Kleurplaten voor Elke Leeftijd</h2>
<p>Onze hoogwaardige verzameling ${title} kleurplaten biedt creatieve inspiratie voor jong en oud. Alle sjablonen zijn strak getekend en geoptimaliseerd voor eenvoudig A4-afdrukken en digitaal kleuren.</p>

<h3>Leeftijdsgroepen & Moeilijkheidsgraden</h3>
<p>Kies de perfecte ${title} kleurplaat op basis van je ervaring en niveau:</p>
<ul>
...
<p>Zoek je naar <em>gratis ${title} kleurplaat printen pdf</em> of <em>makkelijke ${title} kleurplaten voor kinderen</em>? Bij ColorVaults download je snel en zonder account.</p>

${relatedLinksHtml ? `<h3>Ontdek ook gerelateerde categorieën</h3>\n<p>Bekijk ook deze populaire categorieën:</p>\n${relatedLinksHtml}` : ''}`;
    }
  }

  return { seoIntro, seoContent };
}

const updatedEn = enThemes.map(t => {
  const { seoIntro, seoContent } = generateSeoForTheme(t, 'en', enThemes);
  return { ...t, seoIntro, seoContent };
});

const updatedNl = nlThemes.map(t => {
  const { seoIntro, seoContent } = generateSeoForTheme(t, 'nl', nlThemes);
  return { ...t, seoIntro, seoContent };
});

fs.writeFileSync(enThemesPath, JSON.stringify(updatedEn, null, 2), 'utf8');
fs.writeFileSync(nlThemesPath, JSON.stringify(updatedNl, null, 2), 'utf8');

console.log(`Successfully populated seoIntro and seoContent in ${updatedEn.length} EN themes and ${updatedNl.length} NL themes!`);
